...
public class Dimentions {
	...
	public static void setDimention(String distanceString, String Units, JPanel DoublePanal) {
		...
		Line L = DrawingObjects.Lines.get(DrawingObjects.SelectedLine);

		double distHypotinuse = Math
				.sqrt(Math.pow(L.Points[1].y - L.Points[0].y, 2) + Math.pow(L.Points[1].x - L.Points[0].x, 2));

		double multiplyer = distance / distHypotinuse;

		L.Points[1].y = (L.Points[0].y + (L.Points[1].y - L.Points[0].y) * multiplyer);
		L.Points[1].x = (L.Points[0].x + (L.Points[1].x - L.Points[0].x) * multiplyer);

		DrawingObjects.Lines.set(DrawingObjects.SelectedLine, L);
	...
	/**
	 * Gets the distance a point is form a line represented as to points
	 * 
	...
	public void mousePressed(MouseEvent e) {
		...
		if (SwingUtilities.isLeftMouseButton(e) && MainWindow.DrawingPanel.getBounds().contains(click)) {
			...
			for (int i = 0; i < DrawingObjects.Lines.size(); i++) {
				if (Dimentions.distanceFromLine(DrawingObjects.Lines.get(i).Points[0],
						DrawingObjects.Lines.get(i).Points[1], click) < 10) {
